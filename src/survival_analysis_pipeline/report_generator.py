...
range, and that range is the interval beside it. The interval is the
range the score stayed inside for 95% of resamples of the test
{c["units"]}. A narrow interval means the score is precise on this test
set. An interval that sits wholly above 0.500 means the model beats a
coin flip even at its low end. The pooled score is not the number for
...
{fig_dep}"""
    ...
    if c["cox_top"]:
        ...
        ref_cols: list[str] = []
        for row in c["cox_top"][:8]:
            col = row["feature"].split("=", 1)[0]
            if "=" in row["feature"] and col in refs and col not in ref_cols:
                ref_cols.append(col)
        ref_sentence = "".join(
            f" For <code>{col}</code> the reference is {refs[col]}." for col in ref_cols
        )
        tab_hr = doc.table(
            "cox-hr",
            ...
            f" independent.{ref_sentence}"
            " The ridge penalty that stabilizes the fit shrinks"
