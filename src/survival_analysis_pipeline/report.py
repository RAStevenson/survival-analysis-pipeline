...
def synthetic_context(m: dict, run_dir: Path, notes_dir: Path | None = None) -> dict:
    ...
    <tr><th>Run</th><td>seed {g["seed"]}, {d["n_rows"]:,} strategies,
      {len(folds)} temporal folds</td></tr>
    <tr><th>Source of figures</th><td><code>{rel}/metrics.json</code>, regenerated
      by <code>python scripts/run_synthetic_pipeline.py</code></td></tr>"""

    footer = f"""<p>Generated from <code>{rel}/metrics.json</code> by
<code>scripts/run_build_report.py</code>. Seed {g["seed"]},
{d["n_rows"]:,} strategies, {m["pooled"]["n_test"]:,} out-of-fold test
strategies.</p>"""
...
<p>The pipeline has no support for left truncation, {c["units"]} whose life
began before the source window. Those must be excluded during preparation,
and whether they were is recorded with the dataset.</p>"""
    if c["g"]:
        g = c["g"]
        body += "\n" + _pk(
            "generator-data",
...
scale was {p["aft_sigma"]}, the calibrated predictive scale
