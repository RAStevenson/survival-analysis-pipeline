...
pipeline, validated on synthetic data against an oracle, and demonstrated 
...
This document includes walkthroughs on reproducing the synthetic data results, 
reproducing the real data demonstration, and how to use this system on your own data. 
...
and some strategies last far longer than others. This project builds a
model pipeline intended to predict a strategy's lifespan from the metadata 
available on the day it is deployed.
...
The full write-up on the synthetic data is in the autogenerated
...
To examine the pipeline findings demonstrated on a real life data set of a
different genre, see the Chicago Business Licence Analysis report included
in this repo and explained at the end of this document.

Reproducing the synthetic results is explained next.

## Quick start - Reproducing Synthetic Results
...
by construction (the special event, pop-up, and itinerant permits) because their
enrichment skews results and tells you nothing about which businesses
intended longer lifespans but did not survive. A temporary permit is always going to
expire, so its short life is intent rather than failure. The analysis asks
