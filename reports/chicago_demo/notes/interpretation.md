...
license types in 2012, the same year Regulated Business License first
...
The measurements in this and the previous paragraph are recomputed by `scripts/run_fold3_investigation.py`.

The printed bootstrap interval is narrower than the real uncertainty.
Licenses in the same category and the same stretch of time tend to fail
