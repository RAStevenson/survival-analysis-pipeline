#!/usr/bin/env python3
"""Build the report from reports/metrics.json and the figures.
...
Run from the repo root after `python -m strategy_survival run`:
...
METRICS_SEED8 = HERE / "metrics_seed8.json"
FIGURES = HERE / "figures"
OUT = HERE / "strategy_survival_report.html"
OUT_PDF = HERE / "strategy_survival_report.pdf"
...
def seed_dependence_para(m: dict, m8: dict | None) -> str:
    ...
    return (
        ...
        f"{'selected the same hyperparameters' if same_params else 'selected different hyperparameters'} "
        ...
        f"at {pool8['c_oracle']:.3f}. " + attribution
...
<header class="titleblock">
  ...
  <table class="meta">
    ...
    <tr><th>Source of figures</th><td>reports/metrics.json, regenerated by
      <code>python -m strategy_survival run</code></td></tr>
  </table>
</header>
...
<figure>
  <img src="{img('shap_dependence.png')}" alt="SHAP dependence plots for the three walk-forward statistics">
...
.venv\\Scripts\\python reports/build_report.py</code></pre>

<p>The test suite is {N_TESTS} tests covering generator invariants, fold and
label leakage, and metric behaviour. The <code>run</code> command regenerates
the data at seed {SEED}, runs the {len(folds)}-fold temporal
cross-validation, and writes <code>reports/metrics.json</code> and all figures
in about two minutes. The final command regenerates this report from that
metrics file, so every number here is traceable to a single run rather than
transcribed.</p>

<p>The seed-dependence paragraph in section 8 is generated from
<code>reports/metrics_seed8.json</code>, produced by
<code>python -m strategy_survival run --seed 8</code> and saved under that name;
...
<code>reports/build_report.py</code>. Seed {SEED},
